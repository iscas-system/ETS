...
class Kernel(object):
    ...
    def setDirection(self):
        ...
        if any("Backward, seq = " in x for x in self.seqMarkers) or \
                ...
                any("Backward0, seq = " in x for x in self.seqMarkers) or\
                any("Backward1, seq = " in x for x in self.seqMarkers):
            self.dir = "bprop"
        else:
            self.dir = "fprop"
    ...
    def setOp(self):
        ...
        def format_autograd(name):
            name = name.replace("torch", "")\
                .replace("autograd", "")\
                .replace("evaluate_function: ", "")\
                .replace("aten", "") \
                .replace("::", "")\
                .replace("engine", "")
            return  name.split(',')[0]
        ...
        for m in self.seqMarkers:
            if ("backward, seq = " in m) or ("Backward, seq = " in m) \
                    or ("Backward0, seq = " in m) or ("Backward1, seq = " in m):
                op = m.split(",")[0]
                op = sanitize(op)
                self.op.append(op)
                self.mod.append('na')
        ...
        if len(self.otherMarkers):
            for m in self.otherMarkers:
                op = m.split(",")[0]
                op = format_autograd(op)
                self.op.append(op)
                self.mod.append('na')
        self.mod.append('na')
