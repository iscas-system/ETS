...
class NVVP(object):
    ...
    def getMarkerInfo(self, objId, startTime, endTime):
        ...
        def opcompare(elem):
            """
            Sorting function for other markers
            """
            assert (", op_id = " in elem)
            #sort by sequence id and then the string
            l = elem.split(" = ")
            return l[1]

        #Remove duplicates, sort and prune layerMarkers
        def prune(mlist):
            """
        ...
        for r in result:
            ...
            if ("_backward, seq =" in m) or ("Backward, seq =" in m) or ("Backward0, seq =" in m) or ("Backward1, seq =" in m) or ('backward, seq =' in m):
                bprop = True
        ...
        if (len(otherMarkers)):
            otherMarkers.sort(key=opcompare)
